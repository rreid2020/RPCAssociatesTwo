export const ENGAGEMENT_ASSIGNMENT_ROLES = new Set(['preparer', 'reviewer', 'manager', 'partner'])

const LEGACY_ENGAGEMENT_ROLE_ALIASES = {
  member: 'partner'
}
...
export function normalizeEngagementAssignmentRole (role) {
  ...
  if (!ENGAGEMENT_ASSIGNMENT_ROLES.has(canonical)) {
...
export function parseEngagementAssignmentsPayload (payload = {}) {
  if (Array.isArray(payload.assignments) && payload.assignments.length > 0) {
    const parsed = payload.assignments.map((entry) => ({
      ...
      assignmentRole: normalizeEngagementAssignmentRole(entry?.assignmentRole || entry?.assignment_role || 'partner')
