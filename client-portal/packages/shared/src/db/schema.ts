...
export const trialBalanceAccounts = taxgptSchema.table('trial_balance_accounts', {
  ...
  adjustmentDebit: numeric('adjustment_debit', { precision: 14, scale: 2 }).notNull().default('0'),
  adjustmentCredit: numeric('adjustment_credit', { precision: 14, scale: 2 }).notNull().default('0'),
  reviewStatus: varchar('review_status', { length: 24 }).notNull().default('needs_work'),
  workpaperNote: text('workpaper_note'),
  createdAt: timestamp('created_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
});
...
export const workingPapers = taxgptSchema.table('working_papers', {
  id: uuid('id').primaryKey().defaultRandom(),
  organizationId: uuid('organization_id'),
  workspaceId: uuid('workspace_id'),
  engagementId: uuid('engagement_id').notNull().references(() => accountingEngagements.id, { onDelete: 'cascade' }),
  leadSheetId: uuid('lead_sheet_id').references(() => leadSheets.id, { onDelete: 'set null' }),
  paperCode: varchar('paper_code', { length: 24 }).notNull(),
  title: text('title').notNull(),
  status: varchar('status', { length: 24 }).notNull().default('not_started'),
  dueDate: date('due_date'),
...
export const workingPaperSections = taxgptSchema.table('working_paper_sections', {
  id: uuid('id').primaryKey().defaultRandom(),
  workingPaperId: uuid('working_paper_id').notNull().references(() => workingPapers.id, { onDelete: 'cascade' }),
...
export const workingPaperRows = taxgptSchema.table('working_paper_rows', {
  id: uuid('id').primaryKey().defaultRandom(),
  organizationId: uuid('organization_id'),
  workspaceId: uuid('workspace_id'),
  engagementId: uuid('engagement_id').notNull().references(() => accountingEngagements.id, { onDelete: 'cascade' }),
  leadSheetId: uuid('lead_sheet_id').notNull().references(() => leadSheets.id, { onDelete: 'cascade' }),
  workingPaperId: uuid('working_paper_id').references(() => workingPapers.id, { onDelete: 'set null' }),
  sectionId: uuid('section_id').references(() => workingPaperSections.id, { onDelete: 'set null' }),
  trialBalanceAccountId: uuid('trial_balance_account_id').notNull().references(() => trialBalanceAccounts.id, { onDelete: 'cascade' }),
  rowLabel: text('row_label').notNull(),
  reconciliationState: varchar('reconciliation_state', { length: 24 }).notNull().default('not_started'),
  reviewStatus: varchar('review_status', { length: 24 }).notNull().default('pending'),
  signoffState: varchar('signoff_state', { length: 24 }).notNull().default('unsigned'),
  assignedPreparerId: text('assigned_preparer_id'),
  assignedReviewerId: text('assigned_reviewer_id'),
  dueDate: date('due_date'),
  sortOrder: integer('sort_order').notNull().default(0),
...
export const journalEntries = taxgptSchema.table('journal_entries', {
  id: uuid('id').primaryKey().defaultRandom(),
  organizationId: uuid('organization_id'),
  workspaceId: uuid('workspace_id'),
  engagementId: uuid('engagement_id').notNull().references(() => accountingEngagements.id, { onDelete: 'cascade' }),
  adjustmentEntryId: uuid('adjustment_entry_id').references(() => adjustmentEntries.id, { onDelete: 'set null' }),
  entryNumber: text('entry_number').notNull(),
  description: text('description').notNull(),
  status: varchar('status', { length: 24 }).notNull().default('draft'),
  source: varchar('source', { length: 24 }).notNull().default('manual'),
...
export const journalEntryLines = taxgptSchema.table('journal_entry_lines', {
  id: uuid('id').primaryKey().defaultRandom(),
  journalEntryId: uuid('journal_entry_id').notNull().references(() => journalEntries.id, { onDelete: 'cascade' }),
  adjustmentEntryLineId: uuid('adjustment_entry_line_id').references(() => adjustmentEntryLines.id, { onDelete: 'set null' }),
  accountNumber: text('account_number'),
  accountName: text('account_name').notNull(),
  debitAmount: numeric('debit_amount', { precision: 14, scale: 2 }).notNull().default('0'),
  creditAmount: numeric('credit_amount', { precision: 14, scale: 2 }).notNull().default('0'),
...
export const tickmarks = taxgptSchema.table('tickmarks', {
  id: uuid('id').primaryKey().defaultRandom(),
  organizationId: uuid('organization_id'),
  workspaceId: uuid('workspace_id'),
  engagementId: uuid('engagement_id').notNull().references(() => accountingEngagements.id, { onDelete: 'cascade' }),
  leadSheetId: uuid('lead_sheet_id').notNull().references(() => leadSheets.id, { onDelete: 'cascade' }),
  workingPaperRowId: uuid('working_paper_row_id').notNull().references(() => workingPaperRows.id, { onDelete: 'cascade' }),
  tickmarkCode: varchar('tickmark_code', { length: 12 }).notNull(),
  label: text('label'),
...
export const evidenceLinks = taxgptSchema.table('evidence_links', {
  id: uuid('id').primaryKey().defaultRandom(),
  organizationId: uuid('organization_id'),
  workspaceId: uuid('workspace_id'),
  engagementId: uuid('engagement_id').notNull().references(() => accountingEngagements.id, { onDelete: 'cascade' }),
  leadSheetId: uuid('lead_sheet_id').notNull().references(() => leadSheets.id, { onDelete: 'cascade' }),
  workingPaperRowId: uuid('working_paper_row_id').references(() => workingPaperRows.id, { onDelete: 'set null' }),
  documentId: uuid('document_id').references(() => workingPaperDocuments.id, { onDelete: 'set null' }),
  linkType: varchar('link_type', { length: 24 }).notNull().default('document'),
  label: text('label'),
...
export const assignmentTracking = taxgptSchema.table('assignment_tracking', {
  id: uuid('id').primaryKey().defaultRandom(),
  organizationId: uuid('organization_id'),
  workspaceId: uuid('workspace_id'),
  engagementId: uuid('engagement_id').notNull().references(() => accountingEngagements.id, { onDelete: 'cascade' }),
...
export const reviewSignoffs = taxgptSchema.table('review_signoffs', {
  id: uuid('id').primaryKey().defaultRandom(),
  organizationId: uuid('organization_id'),
  workspaceId: uuid('workspace_id'),
  engagementId: uuid('engagement_id').notNull().references(() => accountingEngagements.id, { onDelete: 'cascade' }),
  leadSheetId: uuid('lead_sheet_id').references(() => leadSheets.id, { onDelete: 'set null' }),
  signoffType: varchar('signoff_type', { length: 24 }).notNull(),
  signoffState: varchar('signoff_state', { length: 24 }).notNull().default('signed'),
...
export const workflowStatuses = taxgptSchema.table('workflow_statuses', {
  id: uuid('id').primaryKey().defaultRandom(),
  organizationId: uuid('organization_id'),
  workspaceId: uuid('workspace_id'),
  engagementId: uuid('engagement_id').notNull().references(() => accountingEngagements.id, { onDelete: 'cascade' }),
  leadSheetId: uuid('lead_sheet_id').references(() => leadSheets.id, { onDelete: 'set null' }),
  workingPaperRowId: uuid('working_paper_row_id').references(() => workingPaperRows.id, { onDelete: 'set null' }),
  statusType: varchar('status_type', { length: 24 }).notNull(),
  statusValue: varchar('status_value', { length: 24 }).notNull(),
...
export const auditEvents = taxgptSchema.table('audit_events', {
  id: uuid('id').primaryKey().defaultRandom(),
  organizationId: uuid('organization_id'),
  workspaceId: uuid('workspace_id'),
  engagementId: uuid('engagement_id').references(() => accountingEngagements.id, { onDelete: 'cascade' }),
  leadSheetId: uuid('lead_sheet_id').references(() => leadSheets.id, { onDelete: 'set null' }),
  workingPaperRowId: uuid('working_paper_row_id').references(() => workingPaperRows.id, { onDelete: 'set null' }),
