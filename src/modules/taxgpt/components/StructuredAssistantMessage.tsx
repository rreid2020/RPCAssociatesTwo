...
function SourceExcerpt ({ excerpt }: { excerpt: string }) {
  return (
    <blockquote className="mt-2 border-l-2 border-primary/30 bg-white/60 px-3 py-2 text-sm leading-relaxed text-text">
      {excerpt}
    </blockquote>
  )
}
    ...
    <div className="space-y-5">
      ...
      <section>
        ...
        <p className="mt-1 text-xs text-text-light">
          Retrieved passages from the knowledge base. Numbered citations such as [1] match the references section below.
        </p>
                  ...
                  <ul className="mt-2 space-y-4">
                    {group.entries.map((entry) => (
                      <li key={entry.id || entry.chunkId || entry.sourceUrl}>
                        ...
                        {entry.excerpt ? (
                          <SourceExcerpt excerpt={entry.excerpt} />
                        ) : entry.summary ? (
